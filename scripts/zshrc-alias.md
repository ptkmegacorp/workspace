...
- `tmux-work` → `tmux -S /tmp/shared-tmux.sock select-window -t shared-tmux:work >/dev/null && tmux -S /tmp/shared-tmux.sock attach -t shared-tmux`
- `tmux-tree` → `tmux -S /tmp/shared-tmux.sock select-window -t shared-tmux:project-tree >/dev/null && tmux -S /tmp/shared-tmux.sock attach -t shared-tmux`
