...
- Added `projects/always-on-mic/bin/pause-trigger.sh` to consume pause events and feed transcripts into `openclaw prompt` along with metadata.
...
2. Layer in safety controls (rate limiting, mute flag, logging) before triggering `projects/always-on-mic/bin/pause-trigger.sh`.
